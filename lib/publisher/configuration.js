 ...
 * no matter whether the OWLHUB_SDK_LOAD_CONFIG variable is set.
 * @api private
 */
...
function fromEnvironment(config) {
  config.port = config.port || process.env.OWLHUB_CSM_PORT;
  config.enabled = config.enabled || process.env.OWLHUB_CSM_ENABLED;
  config.clientId = config.clientId || process.env.OWLHUB_CSM_CLIENT_ID;
  config.host = config.host || process.env.OWLHUB_CSM_HOST;
...
function fromConfigFile(config) {
  ...
  try {
    var configFile = OWLHUB.util.iniLoader.loadFrom({
      isConfig: true,
