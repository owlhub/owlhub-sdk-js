  ...
  coalesceRefresh: function coalesceRefresh(callback, sync) {
    ...
    if (self.refreshCallbacks.push(callback) === 1) {
      self.load(function onLoad(err) {
        OWLHUB.util.arrayEach(self.refreshCallbacks, function(callback) {
          if (sync) {
            callback(err);
          } else {
            // callback could throw, so defer to ensure all callbacks are notified
            OWLHUB.util.defer(function () {
              callback(err);
            });
          }
        });
 ...
 * @api private
 */
OWLHUB.Credentials.addPromisesToClass = function addPromisesToClass(PromiseDependency) {
  this.prototype.getPromise = OWLHUB.util.promisifyMethod('get', PromiseDependency);
  this.prototype.refreshPromise = OWLHUB.util.promisifyMethod('refresh', PromiseDependency);
};

/**
 * @api private
 */
OWLHUB.Credentials.deletePromisesFromClass = function deletePromisesFromClass() {
  delete this.prototype.getPromise;
  delete this.prototype.refreshPromise;
};

OWLHUB.util.addPromises(OWLHUB.Credentials);
