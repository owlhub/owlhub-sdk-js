...
var serviceClasses = {};
Object.keys(OWLHUB).forEach(function(name) {
  if (OWLHUB[name].serviceIdentifier) {
    serviceClasses[OWLHUB[name].serviceIdentifier] = OWLHUB[name];
  }
});
...
function getServiceHeader(service) {
  ...
  var file = util.format(
    'if (!Object.prototype.hasOwnProperty.call(OWLHUB, \'' + metadata[service].name + '\')) {\n' +
    '  OWLHUB.apiLoader.services[\'%s\'] = {};\n' +
    '  OWLHUB.%s = OWLHUB.Service.defineService(\'%s\', %s);\n',
...
function getService(service, version) {
  ...
  var line = util.format(
    'OWLHUB.apiLoader.services[\'%s\'][\'%s\'] = %s;\n',
  ...
  if (Object.prototype.hasOwnProperty.call(api, 'paginators') && Object.keys(api.paginators).length) {
    line = util.format(
      'OWLHUB.apiLoader.services[\'%s\'][\'%s\'].paginators = %s;\n',
  ...
  if (Object.prototype.hasOwnProperty.call(api, 'waiters') && Object.keys(api.waiters).length) {
    line = util.format(
      'OWLHUB.apiLoader.services[\'%s\'][\'%s\'].waiters = %s;\n',
